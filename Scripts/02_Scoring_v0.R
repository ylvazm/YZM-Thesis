...
#' ---
#' title: "Scoring of TIMSS 2023 Grade 8 Achievement Data - Scoring only the Valid Item Responses"
#' author: "Ylva Matejko"
#' date: "Mar 19, 2026"
#' ---

#' This script creates a TIMSS 2023 grade 8 response dataset where the original codes for omitted and not-reached items are kept as-is, while valid item responses are scored. 
#' It saves the resulting data.frame as GRADE8DATA_COMPLETE_v0_SCR.Rdata.
#' 
...
doit <- function(indir=getwd(), outdir=getwd(), infile=""){
    output_file <- paste0(outdir,infile,"_SCR_v0.Rdata") # file name adjusted
    ...
    # Save file
    env <- new.env()
    data_name <- paste0(infile,"_SCR_v0") # file name adjusted
    env[[data_name]] <- data
