#' ---
#' title: "Scoring of TIMSS 2023 Grade 8 Achievement Data - Scoring Omitted Items as Incorrect"
#' author: "Ylva Matejko"
#' date: "Mar 05, 2025"
#' ---

#' This script creates a TIMSS 2023 grade 8 response dataset where omitted items are scored as incorrect. 
#' It saves the resulting data.frame as GRADE8DATA_COMPLETE_v2_SCR.Rdata.
#' 
...
#' and scoring algorithm to achieve the desired handling of item omissions.
#' 
...
scoreit <- function(data, item, type, right, NR, OM, other){
  ...
  if  (toupper(type)=="MC"){
    data <- data %>% mutate_at(vars(item), ~case_when ( . == right ~ 1,   # Item Key
                                                        . == NR    ~ NaN, # Not Reached
                                                        . == OM    ~ 0, # Omitted - this line was adjusted
  ...
  if  (toupper(type)=="CR"){
    data <- data %>% mutate_at(vars(item), ~case_when ( . ==2 | (. >= 20 & . < 30)  ~ 2, # Full Credit
                                                        ...
                                                        . %in% OM ~ 0,    # Omitted - this line was adjusted
                                                        is.na(.) ~ NaN,   # Not Administered
...
doit <- function(indir=getwd(), outdir=getwd(), infile=""){
  output_file <- paste0(outdir,infile,"_SCR_v2.Rdata") # file name adjusted
  ...
  # Save file
  env <- new.env()
  data_name <- paste0(infile,"_SCR_v2") # file name adjusted
  env[[data_name]] <- data
