...
#' ---
#' title: "Scoring of TIMSS 2023 Grade 8 Achievement Data -  Scoring Omitted Items as Not Administered"
#' author: "Ylva Matejko"
#' date: "Mar 05, 2026"
#' ---

#' This script creates a TIMSS 2023 grade 8 response dataset where omitted items are treated as not administered. 
#' It saves the resulting data.frame as GRADE8DATA_COMPLETE_v1_SCR.Rdata.
#' 
...
doit <- function(indir=getwd(), outdir=getwd(), infile=""){
    output_file <- paste0(outdir,infile,"_SCR_v1.Rdata") # file name adjusted
    ...
    # Save file
    env <- new.env()
    data_name <- paste0(infile,"_SCR_v1") # file name adjusted
    env[[data_name]] <- data
